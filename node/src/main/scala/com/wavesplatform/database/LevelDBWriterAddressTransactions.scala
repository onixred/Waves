    ...
    db.get(Keys.addressId(address)).fold(Observable.empty[(Height, Transaction)]) { id =>
      val (heightAndTxs, closeF) = if (dbSettings.storeTransactionsByAddress) {
        ...
        (takeAfter(takeTypes(heightNumStream, types.map(_.typeId)), maybeAfter)
           .flatMap { case (height, _, txNum) => db.get(Keys.transactionAt(height, txNum, levelDBWriter.estimator())).map((height, txNum, _)) },
         () => ())
      } else {
        ...
        .flatMap {
          case (height, txNum) =>
            db.get(Keys.transactionBytesAt(height, txNum))
              .flatMap { txBytes =>
                if (ofTypes.isEmpty)
                  TransactionParsers.parseBytes(txBytes, levelDBWriter.estimator()).toOption
                else {
                  ofTypes.iterator
                    .map(_.parseBytes(txBytes, levelDBWriter.estimator()))
