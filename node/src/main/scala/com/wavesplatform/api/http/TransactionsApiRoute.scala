...
case class TransactionsApiRoute(
  ...
  def signWithSigner: Route = pathPrefix(Segment) { signerAddress =>
    jsonPost[JsObject](TransactionFactory.parseRequestAndSign(wallet, signerAddress, time, _, blockchain.estimator()))
  }
  ...
  def signedBroadcast: Route = path("broadcast")(broadcast[JsValue](TransactionFactory.fromSignedRequest(_, blockchain.estimator())))
